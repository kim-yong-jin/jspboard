...
public class ServletFileUploadBuilder {
	...
	public static ServletFileUpload build(int MEMORY_THRESHOLD, int MAX_FILE_SIZE, int MAX_REQUEST_SIZE) {
		...
		DiskFileItemFactory factory = new DiskFileItemFactory();               //실제 파일을 저장해주는애를 모아둔 팩토리
		
		// 저장을 위한 threshold memory 적용
		factory.setSizeThreshold(MEMORY_THRESHOLD);
		
		// 임시 저장 위치 결정
		factory.setRepository(new File(System.getProperty("java.io.tmpdir"))); // 임시 저장은 요기다 하겠다는거지?
